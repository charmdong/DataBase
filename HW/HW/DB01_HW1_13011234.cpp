...
void inputFile(char* fname)	
{
	...
	while (1) {		// 파일명이 Student.txt 과 일치할 때까지 반복
		scanf("%s", fname);
...
{
	*cnt = 0;	// 저장된 데이터의 갯수를 0으로 초기화
	while (!feof(fp))	// 파일의 끝까지 반복
	{
		// 데이터가 완전하지 않은 경우 오류문 출력 이후 반복문 탈출
...
{
	char studentInfo[MAX];	// 새로운 학생 정보를 입력 받기 위한 문자열 
...
{
	int i;
	int checkCnt = 1;	// 찾고자 하는 데이터 여부를 확인하는 변수
	int choice;				// 메뉴 선택 변수
	char search[20];	// 찾고자 하는 학번, 이름, 학년, 학점 중 하나 입력 받을 문자열

	printf("\n1.No\n2.Name\n3.Year\n4.Grade\nSELECT >> ");
	while (1) {
		if (scanf("%d", &choice) && choice > 0 && choice < 5)	// 1~4 사이의 수를 입력받으면 반복문 탈출
			break;
		printf("  >>  ");
	}
	...
	case 1: for (i = 0; i < *cnt; i++) {
					if (strcmp(search, ary[i].stuNum) == 0) {	// 일치하는 데이터가 있는 경우, 해당하는 학생에 대한 정보 출력 및 checkCnt를 0으로 변경
	...
	case 2: for (i = 0; i < *cnt; i++) {
					if (strcmp(search, ary[i].name) == 0) {		// 일치하는 데이터가 있는 경우, 해당하는 학생에 대한 정보 출력 및 checkCnt를 0으로 변경
						printf("%s %s %s %s\n", ary[i].stuNum, ary[i].name, ary[i].year, ary[i].grade);
						checkCnt = 0;
					}
				}
	...
	case 3: for (i = 0; i < *cnt; i++) {
					if (strcmp(search, ary[i].year) == 0) {			// 일치하는 데이터가 있는 경우, 해당하는 학생에 대한 정보 출력 및 checkCnt를 0으로 변경
						printf("%s %s %s %s\n", ary[i].stuNum, ary[i].name, ary[i].year, ary[i].grade);
						checkCnt = 0;
					}
				}
	...
	case 4: for (i = 0; i < *cnt; i++) {
					if (strcmp(search, ary[i].grade) == 0) {		// 일치하는 데이터가 있는 경우, 해당하는 학생에 대한 정보 출력 및 checkCnt를 0으로 변경
						printf("%s %s %s %s\n", ary[i].stuNum, ary[i].name, ary[i].year, ary[i].grade);
						checkCnt = 0;
					}
				}
